...
function miniPc() {
  ...
  g.add(field);
  /*
   * The ribs face straight up into the studio ceiling, and at metalness 0.68 /
   * roughness 0.36 they mirrored it: nine bright bars in a pale recess, which
   * at card size read as a white sticker on the lid. The cover beside them
   * already carries envMapIntensity 0.35 for exactly this reason; the ribs
   * were left out of it. Same treatment, plus enough roughness that they are
   * vents catching a sheen rather than mirrors.
   */
  const rib = mat({
    color: 0x12171f,
    metalness: 0.5,
...
export function buildSubject(name, machine, dressScreens = () => {}) {
  ...
  if (name === "cat-desktops") {
    ...
    /* Two objects staged as one mass, not two things at opposite ends of the
       frame. A 33 cm tower beside an 18 cm mini is mostly air however the
       bounding box is fitted: spread them and the box fills the card while the
       objects inside it shrink — measured, 43 cm apart put the subject on
       16.6% of the pixels and 60 cm apart put it on 14.6%. So the mini comes in
       close and forward instead, where perspective makes it large and it
       overlaps the tower's foot. */
    const t = tower();
    t.position.set(-cm(6), 0, -cm(3));
    g.add(t);
    /*
     * The mini stands on its side, in its stand, which is both how these are
     * deployed in an office and the only way this card works.
     *
     * Lying flat it presented a 18 x 18 cm horizontal face to a directional
     * key set six stops above the near-black verticals around it, and that
     * face came back a flat cream slab with the vent recess printed on it —
     * the brightest thing on a card whose subject is a dark tower. Chased
     * through three materials first: envMapIntensity 0.35 to 0.12, metalness
     * 0.3 to 0.22 to 0.55, roughness 0.72 to 0.82 to 0.42. A magenta test
     * render settled it — the whole lid came back saturated, so it is not a
     * reflection of anything, it is the key on a face pointing straight at it.
     * Nothing in the material can fix a surface normal aimed at the lamp.
     *
     * Upright, its large faces are vertical like the tower's and it reads as
     * the same hardware in the same room. Local contrast is the price and the
     * crop pays it: at fit 1.10 the pair measured 16.9 and the checker called
     * mush, at 0.88 it measured 18.2, and 0.95 is the top of that curve at
     * 18.5.
     */
    const m = miniPc();
    m.position.set(cm(17), cm(9), cm(6));
    m.rotation.set(0, -0.38, Math.PI / 2);
    g.add(m);
    return { object: g, yaw: 0.86, pitch: 0.18, fov: 32, fit: 0.95 };
  }
