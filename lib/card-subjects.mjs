...
/* The hero's stone desk, sampled: #1e2938. A near-black desk in a near-black
   room gives a bench of laptops nothing to sit on. */
const STONE = { color: 0x1a2331, metalness: 0.1, roughness: 0.72 };
...
function screen(w, h) {
  const m = new THREE.Mesh(
    ...
    mat({ ...GLASS, emissive: 0xffffff, emissiveMap: sheen(), emissiveIntensity: 1.9 }),
  );
  m.receiveShadow = true;
  return m;
}
...
export function buildSubject(name, machine, dressScreens = () => {}) {
  ...
  if (name === "cat-monitors") {
    ...
    g.add(monitor());
    const k = keyboard();
    k.position.set(-cm(4), 0, cm(27));
    g.add(k);
    const d = dock();
    d.position.set(cm(30), 0, cm(23));
    d.rotation.y = -0.22;
    g.add(d);
    return { object: g, yaw: 0.44, pitch: 0.26, fov: 26, fit: 1.18, lift: 0.1 };
  }
  ...
  if (name === "cat-desktops") {
    ...
    const t = tower();
    t.position.set(-cm(9), 0, -cm(4));
    g.add(t);
    const m = miniPc();
    m.position.set(cm(16), 0, cm(15));
    m.rotation.y = -0.38;
    g.add(m);
    return { object: g, yaw: 0.62, pitch: 0.27, fov: 30, fit: 1.0 };
  }
  ...
  if (name === "fleet-scene") {
    ...
    return { object: g, yaw: 0.5, pitch: 0.24, fov: 32, fit: 1.0 };
  }
